...
namespace
{
  std::set<Move> toMoves(const MoveStack* begin, const MoveStack* end)
  {
    std::set<Move> moves;
    while (begin != end) {
      moves.insert(begin->move);
      ++begin;
    }
    return moves;
  }
...
TEST_F(GenerateMovesTest, generateDropMoves_verify) {
  ...
  for (int i = 2; i <= sfen.size(); ++i) {
    ...
    MoveStack* moveStakExpectedEnd = pos.turn() == Black ?
      generate_moves::generateDropMoves20151211<Black>(moveStackExpected, pos, pos.emptyBB()) :
      generate_moves::generateDropMoves20151211<White>(moveStackExpected, pos, pos.emptyBB());
    MoveStack moveStackActual[1024];
    MoveStack* moveStakActualEnd = pos.turn() == Black ?
      generate_moves::generateDropMoves<Black>(moveStackActual, pos, pos.emptyBB()) :
      generate_moves::generateDropMoves<White>(moveStackActual, pos, pos.emptyBB());

    EXPECT_EQ(
      toMoves(moveStackExpected, moveStakExpectedEnd),
      toMoves(moveStackActual, moveStakActualEnd));
  }
}

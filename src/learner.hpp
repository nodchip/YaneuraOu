...
public:
  void learn(Position& pos, std::istringstream& ssCmd) {
    eval_ = std::make_unique<Evaluater>();
...
private:
  ...
  void learnParse2(Position& pos) {
    for (int step = 1; step <= stepNum_; ++step) {
      ...
      double endClockSec = clock() / double(CLOCKS_PER_SEC);
      std::cout << "parse2 1 step elapsed: " << (endClockSec - startClockSec) << "[sec]" << std::endl;
      print();
    }
  }
  void print() {
  ...
  std::unique_ptr<Evaluater> eval_;
  int stepNum_;

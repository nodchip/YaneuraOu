    ...
    for (pvIdx = 0; pvIdx < pvSize && !signals.stop; ++pvIdx) {
      ...
      if (5 <= depth && abs(rootMoves[pvIdx].prevScore_) < ScoreKnownWin) {
        ...
        static constexpr double WEIGHTS[] = {
          -0.00800641905516386030,
          -0.00910913851112127300,
          -0.01658130809664726300,
          0.02956526167690753900,
          -0.04738612845540046700,
          0.02711922675371170000,
          -0.03595404699444770800,
          0.06474193930625915500,
          -0.02717879787087440500,
          0.01829698123037815100,
          -0.01530614029616117500,
          -0.10751610994338989000,
          0.02228296734392643000,
          0.04815026745200157200,
          0.09066718816757202100,
          0.98180192708969116000,
        };
        static constexpr int NUMBER_OF_VARIABLES = sizeof(WEIGHTS) / sizeof(WEIGHTS[0]);
        if (depth > NUMBER_OF_VARIABLES) {
          double sum = -0.00183092406950891020;

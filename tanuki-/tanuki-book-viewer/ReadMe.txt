================================================================================
    MFC (MICROSOFT FOUNDATION CLASS) ライブラリ : tanuki-book-viewer プロジェクトの概要
===============================================================================

この tanuki-book-viewer アプリケーションは、アプリケーション ウィザードによって作成されました。このアプリケーションは MFC の基本的な使い方を示すだけでなく、アプリケーションを作成するための開始点でもあります。

このファイルには、tanuki-book-viewer アプリケーションを構成する各ファイルの内容の概要が含まれています。

tanuki-book-viewer.vcxproj
...
tanuki-book-viewer.vcxproj.filters
...
tanuki-book-viewer.h
    これはアプリケーションのメイン ヘッダー ファイルです。
    その他のプロジェクト固有のヘッダー (Resource.h など) が含まれ、CtanukibookviewerApp アプリケーション クラスを宣言します。

tanuki-book-viewer.cpp
    これはメインのアプリケーション ソース ファイルで、CtanukibookviewerApp アプリケーション クラスが含まれています。

tanuki-book-viewer.rc
...
res\tanuki-book-viewer.ico
    これはアプリケーションのアイコンとして使用されるアイコン ファイルです。このアイコンは、メイン リソース ファイル tanuki-book-viewer.rc にインクルードされます。

res\tanukibookviewer.rc2
...
tanuki-book-viewerDlg.h, tanuki-book-viewerDlg.cpp - ダイアログ
    これらのファイルには、CtanukibookviewerDlg クラスが含まれます。このクラスはアプリケーションのメイン ダイアログの動作を定義します。ダイアログのテンプレートは、Microsoft Visual C++ で編集できる tanuki-book-viewer.rc に含まれています。
...
StdAfx.h, StdAfx.cpp
    これらのファイルは、tanuki-book-viewer.pch という名前のプリコンパイル済みヘッダー (PCH) ファイルと、StdAfx.obj という名前のプリコンパイル済みの型ファイルをビルドするために使用されます。

Resource.h
    これは、新しいリソース ID を定義する標準のヘッダー ファイルです。このファイルの読み込みおよび更新は、Microsoft Visual C++ で行います。

tanuki-book-viewer.manifest
